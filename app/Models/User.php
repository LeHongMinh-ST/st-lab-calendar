...
class User extends Authenticatable
{
    ...
    {
        if ($this->is_admin) return ' <span class="badge bg-info bg-opacity-10 text-info">Siêu quản trị</span>';
        return '<span class="badge bg-info bg-opacity-10 text-info">'. $this->role?->name().'</span>';
    }

    public function getIsAdminTextAttribute()
    {
        if ($this->is_admin) return ' <span class="badge bg-success bg-opacity-20 text-success"><i class="ph-check"></i></span>';
        return '';
    }
